...
studypal-reels/
  ...
  src/
    ...
    posts/
      types.ts                 PostTemplateDef - fields, color slots, defaults, colorsFromPalette, component
      registry.ts              postTemplates[] - every template the Post Creator + "Post" still know about
      PostStill.tsx            The "Post" <Still> renderStill captures (delayRender-backed hold())
      PostReady.tsx            hold()/release context, asset + color helpers shared by templates
      useFitFontSize.ts        Shrink-to-fit headline (handles bottom-aligned/upward overflow)
      fonts.ts                 Bricolage Grotesque / Figtree / Noto Sans Sinhala via @remotion/google-fonts
      templates/               One file per post design, ported from post-templates/*.html
...
per design, each described by a `PostTemplateDef` in `src/posts/registry.ts`. The GUI previews the component
directly in the DOM, scaled with a CSS transform, and `POST /api/posts/render` (`server/postRenderer.ts`) renders
the same component through the `"Post"` `<Still>` in `Root.tsx` with `renderStill`, so preview and PNG
can't drift apart. Things worth knowing:

- **The bundle is cached** across renders and rebuilt only when a file under `src/` has a newer mtime.
  `POST /api/posts/warm` (called when the page opens) starts it early.
- **Uploaded images are inlined as `data:` URIs** before rendering, because `bundle()` snapshots `assets/`
